...
Worker.prototype.runServer = function (config) {
    ...
    var proxyErrorHandler = function (err, req, res) {
        if (err.code === 'ECONNREFUSED' ||
            ...
            if (req.meta.backendLen > 1) {
                this.router.markDeadBackend(req.meta);
            }
            if (req.error) {
                err = req.error;
                // Clearing the error
    ...
    var httpRequestHandler = function (req, res) {
        ...
        (function () {
            ...
            res.writeHead = function (statusCode) {
                ...
                var startErrorCode = (config.server.deadBackendOn500 === true &&
                    this.router.passiveCheck === false) ? 500 : 501;
        ...
        var proxyRequest = function () {
            this.router.getBackendFromRequest(req, function (err, code, backend) {
                if (err) {
    ...
    var wsRequestHandler = function (req, socket, head) {
        this.router.getBackendFromHostHeader(req.headers.host, function (err, code, backend) {
            if (err) {

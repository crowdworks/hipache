(function () {
    ...
    Config.prototype.prepareRollout = function (rollout) {
        if (typeof rollout !== 'undefined') {
            if (!(rollout.driver instanceof Array)) {
                rollout.driver = [rollout.driver];
            }
        } else {
            console.warn('Missing configuration for the Rollout plugin.');
        }

(function() {
    ...
    var RandomBackendForHost = function RandomBackendForHost(lru, driver, options) {
        this.client = driver;
        this.lru = lru;
        this.options = options || {};

        var logHandler = this.options.logHandler || console.log;
        var debugHandler = this.options.debugHandler || console.log;
    ...
    RandomBackendForHost.prototype.withDebugLogging = function (targetCallback) {
        if (this.options.debug) {
            return function(message, code, backend) {
              var formattedArgs = JSON.stringify({message: message, code: code, backend: backend});
                console.warn("RandomBackendForHost: " + formattedArgs);
                return targetCallback(message, code, backend);
            };
        }
    ...
    RandomBackendForHost.prototype.getBackendFromHostHeader = function (host, callback, lru, client) {
        callback = this.withDebugLogging(callback);
        if (host === undefined) {
